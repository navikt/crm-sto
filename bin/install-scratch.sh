...
insertingTestData() {
   sf data import tree --plan dummy-data/data/plan.json || { error $? '"sf data import tree" command failed.'; }
   sf data import tree --plan dummy-data/sto/plan.json || { error $? '"sf data import tree" command failed.'; }
